...
class GoodTimeControl(QtCore.QObject):
    ...
    def dataRetriever(self):
        ...
        np.save("device2.npy", Device2)
        np.save("device3.npy", Device3)
        np.save("device5ana.npy", Device5ana)
...
# device2 = np.load("device2.npy")
# print(np.max(device2), np.min(device2))

# device3 = np.load("device3.npy")
# print(np.max(device3), np.min(device3))

device5 = np.load("device5ana.npy")
print(np.max(device5), np.min(device5))
print(device5[:, 0])
